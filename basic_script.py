...
def choose_random_centroids(samples, n_clusters, seed=None):
    ...
    random_indicies = tf.random_shuffle(tf.range(0, n_samples), seed=seed)
    begin = [0,]
    size = [n_clusters]
    size[0] = n_clusters
    centroid_indicies = tf.slice(random_indicies, begin, size)
    initial_centroids = tf.gather(samples, centroid_indicies)
